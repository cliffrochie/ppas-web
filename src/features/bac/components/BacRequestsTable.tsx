...
const TABLE_COLUMNS = [
  ...
  { key: 'requester_name', label: 'Requestor' },
...
export const BacRequestsTable = ({
  ...
  const columns = [
    ...
    columnHelper.display({
      id: 'section_name',
      header: 'Section',
      cell: ({ row }) => row.original.requesting_office?.name ?? '—',
    }),
    ...
    <>
      <div className="divide-y divide-border sm:hidden">
        {data.map((request) => {
          ...
          const requestor = r ? `${r.first_name} ${r.last_name}` : '—';

          return (
            <Link
              key={request.id}
              to={`/bac/requests/${request.id}`}
