...
interface BacReviewPanelProps {
  request: PurchaseRequest;
}

const BacReviewPanel = ({ request }: BacReviewPanelProps) => {
  ...
  const handleApprove = () => {
    mutate({ status: 'for_budget_approval', remarks: remarks.trim() || undefined });
  };
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-white">
      <div className="border-b border-gray-200 px-4 py-3 sm:px-6 sm:py-4">
        <h2 className="text-xs font-semibold uppercase tracking-wide text-gray-500">BAC Review</h2>
      </div>
...
interface BacRequestDetailProps {
  request: PurchaseRequest;
}

export const BacRequestDetail = ({ request }: BacRequestDetailProps) => {
