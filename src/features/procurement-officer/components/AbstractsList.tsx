...
export const AbstractsList = () => {
  ...
  const { data: response, isLoading, isError } = useAbstractsOfQuotation(filters);
    ...
    return (
      <div className="min-h-full p-4 sm:p-6">
        <div className="rounded-xl bg-white p-8 text-center shadow-sm">
          <p className="text-sm text-destructive">Failed to load abstracts of quotation. Please try again.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-full p-4 sm:p-6">
      <div className="rounded-xl bg-white shadow-sm">
        <div className="flex items-center justify-between border-b border-gray-100 px-5 py-4">
          <h2 className="text-base font-semibold text-gray-800">Abstracts of Quotation</h2>
          {canManage && <AbstractCreateForm />}
        </div>
        ...
        <div className="px-5 py-4">
          <div className="relative max-w-sm">
            ...
            <Input
              ...
              className="pl-8"
              aria-label="Search abstracts of quotation"
            />
          </div>
        </div>

        <div className="px-2">
          <AbstractsTable data={response?.data ?? []} isLoading={isLoading} />
        </div>
