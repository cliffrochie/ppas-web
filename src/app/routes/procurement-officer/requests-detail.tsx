...
import { ProcurementRequestDetail } from '@/features/procurement-officer/components/ProcurementRequestDetail';

const ProcurementRequestDetailPage = () => {
    ...
    <div className="min-h-full p-4 sm:p-6">
      <div className="flex items-center justify-between gap-4">
        <nav className="flex items-center gap-2 text-sm text-gray-500" aria-label="Breadcrumb">
          <Link to="/procurement-officer/requests" className="hover:text-gray-900">
            Requests
          </Link>
        ...
        <Button asChild className="bg-green-700 text-white hover:bg-green-800">
          <Link to="/procurement-officer/requests">Back</Link>
        </Button>
      </div>
      ...
      <div className="mt-5">
        ...
        {isError && !isLoading && (
          <div className="rounded-lg bg-white p-8 text-center shadow-sm">
            <p className="text-sm text-destructive">
              ...
              <Link to="/procurement-officer/requests" className="underline">
                return to Requests
              </Link>
              .
            </p>
        ...
        {!isLoading && !isError && response?.data && (
          <ProcurementRequestDetail request={response.data} />
        )}
      </div>
    </div>
  );
